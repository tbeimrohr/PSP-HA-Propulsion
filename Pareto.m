...
% 
% tb_factor = 1 - ((abs((tb_ref - (tb1+tb2))/(tb1+tb2)))/tb_ref)^.25
% mpl_factor = (abs((mpl_ref - (mpl))/(mpl)))/mpl_ref
% alt_factor = 1 - (abs((alt_ref - (alt))/(alt)))/alt_ref
% mp_factor = 1 - (abs((mass_tot - (mp))/(mp)))/mass_tot
% maxQ_factor = 1 - (abs((maxQ_ref - (maxQ))/(maxQ)))/maxQ_ref
